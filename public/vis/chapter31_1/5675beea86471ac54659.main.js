    ...
    function(t, e, n) {
      ...
      function f(t, e, n, jQuery) {
        var c = i(n),
          ...
          M = void 0;
        if (!window.isMobile) {
         var first = parseInt(t.time[0].match(/^..../)[0]);
         var last = parseInt(t.time.slice(-1)[0].match(/^..../)[0]);
         var year;
         f.tick.values = [];
         for (year = first; year <= last; year++) f.tick.values.push(''+year+'-01-01');
        }
        try {
          for (
        ...
        var H = l.bb.generate({
          ...
          tooltip: {
            format: {
             title: function(x) { return x.getFullYear()+"-"+(x.getMonth()+1).toString().padStart(2,"0")+"-"+x.getDate().toString().padStart(2,"0"); },
