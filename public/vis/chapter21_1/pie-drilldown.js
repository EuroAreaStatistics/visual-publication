...
window.fetchDataSet(1).then(function (dataset2) {
  ...
  var latest2 = Object.keys(dataset2.dimension.time.category.index).sort().reverse().slice(0, 2);
  ...
  function getSeries(dataset, key, dataset2, key2) {
    return Object.keys(dataset2.dimension.coicop.category.index)
          .sort()
          .map(function (k) {
            key.coicop = k;
            key2.coicop = k;
            var v = getLatestValue(dataset, key);
            var v2 = getLatestValue(dataset2, key2);
            return {
              name: _("vis.product." + k + ".label"),
              drilldown: dataset.extension.drilldown[k],
              y: (v2[0] === null || v2[0] === 0) ? null : v2[0] / 10,
              custom: { date: v[1] === null ? null : ', '+v[1], value : v[0] },
            };
          })
  ...
  function drawChart(geo) {
    ...
    var data = [
      {
        name: _("vis.title"),
        colorByPoint: true,
        data: getSeries(dataset, key, dataset2, key2),
      },
    ];
    ...
    var chart = new Highcharts.Chart({
        ...
        events: {
          ...
          drilldown: function (e) {
            ...
            if (!e.seriesOptions) {
              ...
              window.fetchDataSet(drilldown+1).then(function (dataset2) {
                var key = {
                  geo: geo,
                  time: latest,
                };
      ...
      plotOptions: {
        pie: {
          dataLabels: {
            ...
            format: "{point.name}: {point.custom.value:.1f}%",
          },
        },
      },
      ...
      tooltip: {
        ...
        headerFormat: '',
        pointFormat:
          '{point.name}: <b>{point.custom.value:.1f}%</b>{point.custom.date}<br/>',
      },
      series: data,
      drilldown: {
        series: [],
